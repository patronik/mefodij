...
- Підтримка об'єктно орієнтованого програмування відстуння 
...
### Функції
Функція ствроюється за допомогою ключового слова "**функція**", за яким слідують параметри в дужках а потім тіло функції. Значення параметрів за замовчуванням можна вказати після імені параметра після опереатора "**=**".
...
```
Виклик функції виконується написанням імені функції, за яким в дужках слідують аргументи функції розділені комою. Аргументи передаються за значенням (копіюються в стек функції), але можуть бути і передані за посиланям. Для передачі за посиланням, перед параметром функції треба вказати символ "**&**". 
```
мем тест = 3;
функція сума(а, б = 3) {
    вихід а + б + тест;
